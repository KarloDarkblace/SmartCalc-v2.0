...
#if QT_VERSION >= \
    0x060200  // don't use QT_VERSION_CHECK here, some moc versions don't understand it
namespace QCP {
...
class QCPLabelPainterPrivate {
 ...
 protected:
  ...
  struct LabelData {
    ...
    QRect
        totalBounds;  // is in a coordinate system where label top left is at (0, 0)
    QRect
        rotatedTotalBounds;  // is in a coordinate system where the label anchor is at (0, 0)
  ...
  int mPadding;
  double
      mRotation;  // this is the rotation applied uniformly to all labels, not the heterogeneous rotation in amCircularRotated mode
...
template <class DataType>
class
    QCPDataContainer  // no QCP_LIB_DECL, template class ends up in header (cpp included below)
{
 public:
...
ItemType* QCustomPlot::itemAt(const QPointF& pos, bool onlySelectable) const {
  ...
  foreach (QCPAbstractItem* item, mItems) {
    ...
    if (!currentItem ||
        (onlySelectable &&
         !currentItem
              ->selectable()))  // we could have also passed onlySelectable to the selectTest function, but checking here is faster, because we have access to QCPAbstractItem::selectable
      continue;
...
void QCPAbstractPlottable1D<DataType>::getDataSegments(
  ...
  if (mSelectable ==
      QCP::
          stWhole)  // stWhole selection type draws the entire plottable with selected style if mSelection isn't empty
  {
    if (selected())
